...
router::post('login', function ($ags) {
    ...
    if (isset($_POST['nNovaSenha'])) {
        if (filter_input(INPUT_POST, 'nEmail', FILTER_VALIDATE_EMAIL)) {
            echo '<script>
            var myModal = new bootstrap.Modal(document.getElementById("modalEmailSuccess"));
                myModal.show();
            </script>';
        } else {
            echo '<script>
            var myModal = new bootstrap.Modal(document.getElementById("modalEmailError"));
                myModal.show();
            </script>';
        }
    }
